...
for j = 1:m
  ...
  if size(X,1) > 5
    try
      ...
      % If convex hull fails (collinear points), use simple min/max
      spreads(j,:) = max(X(:,1))-min(X(:,1));
    end
  end
end
...
treedata.CrownDiamMax = L;
  ...
  % If convex hull fails, use simple bounding box
  treedata.CrownDiamAve = max(X(:,1))-min(X(:,1));
  treedata.CrownDiamMax = treedata.CrownDiamAve;
  treedata.CrownAreaConv = (max(X(:,1))-min(X(:,1)))*(max(X(:,2))-min(X(:,2)));
  treedata.CrownAreaAlpha = treedata.CrownAreaConv;
end
